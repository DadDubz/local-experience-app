class CatchReportService {
    ...
    static async getLocationStats(locationId) {
        const locationReports = this.reports.filter(report => report.location === locationId);
    ...
    static calculateAverageSizes(reports) {
        ...
        return Object.entries(sizesBySpecies).reduce((averages, [species, sizes]) => {
            averages[species] = {
    ...
    static analyzeBestTimes(reports) {
        ...
        const hourlySuccess = Array(24).fill(0).map((_, hour) => {
