...
const IPACService = require('../services/ipacService');
...
router.get('/', validateCoordinates, async (req, res) => {
    try {
        ...
        const lands = await PublicLandsService.getAllPublicLands(latitude, longitude, radius);
        res.json(lands);
    } catch (error) {
...
router.get('/fishing', validateCoordinates, async (req, res) => {
    try {
        const { latitude, longitude, radius } = req.coordinates;
        const fishingLocations = await FishingService.getFishingLocations(latitude, longitude, radius);
...
router.get('/fishing/:id', async (req, res) => {
    try {
        const details = await FishingService.getFishingLocationDetails(req.params.id);
        res.json(details);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
router.get('/recreation/:id', async (req, res) => {
    try {
        const details = await PublicLandsService.getRecreationAreaDetails(req.params.id);
        res.json(details);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// GET species data from IPAC
router.get('/species', validateCoordinates, async (req, res) => {
    try {
        const { latitude, longitude, radius } = req.coordinates;
        const speciesData = await IPACService.getSpeciesData(latitude, longitude, radius);
...
router.get('/species/:id', async (req, res) => {
    try {
        const speciesDetails = await IPACService.getSpeciesDetails(req.params.id);
...
router.get('/search', validateCoordinates, async (req, res) => {
    try {
        ...
        switch(type) {
            case 'national':
                results = await PublicLandsService.getNationalLands(latitude, longitude, radius);
                break;
            case 'state':
                results = await PublicLandsService.getStateLands(latitude, longitude, radius);
                break;
            case 'local':
                results = await PublicLandsService.getLocalLands(latitude, longitude, radius);
                break;
            default:
                results = await PublicLandsService.getAllPublicLands(latitude, longitude, radius);
        }
...
router.get('/amenities', validateCoordinates, async (req, res) => {
    try {
        const { latitude, longitude, radius } = req.coordinates;
        const amenities = await PublicLandsService.getNearbyAmenities(latitude, longitude, radius);
        res.json(amenities);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
router.get('/health', (req, res) => {
    res.json({
        ...
        endpoints: {
            ...
            species: '/api/lands/species',
            speciesDetails: '/api/lands/species/:id',
