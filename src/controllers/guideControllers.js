        ...
        try {
            ...
            if (date) {
                guides = guides.filter(guide => 
                    !guide.availability.blackoutDates.includes(new Date(date))
                );
            }
        ...
        try {
            ...
            const stats = {
                ...
                totalRevenue: bookings.reduce((sum, booking) => 
                    sum + (booking.payment.status === 'paid' ? booking.payment.amount : 0), 0
                ),
                popularServices: guide.services.map(service => ({
                    name: service.name,
